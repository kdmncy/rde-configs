...
target/profiles/guix: target/profiles rde/channels-lock.scm
	...
	-C rde/channels-lock.scm -p ${GUIX_PROFILE} \
	${PULL_EXTRA_OPTIONS}

target/profiles/guix-local: target/profiles rde/channels-lock-local.scm
	guix pull --substitute-urls="${SUBSTITUTE_URLS}" \
	-C rde/channels-lock-local.scm -p ${GUIX_PROFILE} \
	${PULL_EXTRA_OPTIONS}

rde/channels-lock.scm: rde/channels.scm
	echo -e "(use-modules (guix channels))\n" > ./rde/channels-lock-tmp.scm
	guix time-machine --substitute-urls="${SUBSTITUTE_URLS}" \
	-C ./rde/channels.scm -- describe -f channels >> ./rde/channels-lock-tmp.scm
	mv ./rde/channels-lock-tmp.scm ./rde/channels-lock.scm

rde/channels-lock-local.scm: rde/channels-local.scm
	echo -e "(use-modules (guix channels))\n" > ./rde/channels-lock-tmp.scm
	guix time-machine --substitute-urls="${SUBSTITUTE_URLS}" \
	-C ./rde/channels-local.scm -- describe -f channels >> ./rde/channels-lock-tmp.scm
